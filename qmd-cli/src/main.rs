...
fn handle_get(
    ...
    let (collection, path) = if is_docid(input_path) {
        ...
        if parts.len() == 2 {
            ...
            return Err(anyhow::anyhow!("Could not resolve path: {input_path}"));
        }
    };
    let doc = store
    ...
    if is_comma_list {
        ...
        for mut doc in matched_docs {
            if doc.body_length > max_bytes {
                doc.body = None;
                results.push((doc, true, Some("File too large".to_string())));
            } else if let Ok(Some(mut full_doc)) =
    ...
    match cmd {
        ...
        ModelCommands::Info { name } => {
            ...
            if model_path.exists() {
                println!("Status: {}", "Downloaded".green());
            } else {
        ...
        ModelCommands::Pull { model, refresh } => {
            use qmd::{
                llm::{DEFAULT_EMBED_MODEL_URI, DEFAULT_RERANK_MODEL_URI},
...
fn handle_qsearch(
    ...
    let search_results: Vec<qmd::SearchResult> = rrf_results
        .iter()
        .map(|r| {
            let parts: Vec<&str> = r
                .file
...
fn handle_rerank(query: &str, files: &str, limit: usize, format: &OutputFormat) -> Result<()> {
    ...
    for file in &file_list {
        let (collection, path) = if is_virtual_path(file) {
            parse_virtual_path(file).unwrap_or((String::new(), file.to_string()))
        } else {
    ...
    match format {
        OutputFormat::Json => {
            let output: Vec<serde_json::Value> = result
                ...
                .iter()
                .take(limit)
                .map(|r| serde_json::json!({"file": r.file, "score": r.score, "rank": r.index + 1}))
                .collect();
